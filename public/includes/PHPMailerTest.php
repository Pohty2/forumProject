 ...
 */
final class PHPMailerTest extends PHPUnit_Framework_TestCase
{
    /**
    ...
    public function testValidate()
    {
        ...
        $unicodeaddresses = [
            'first.last@bÃ¼cher.ch',
            'first.last@ÐºÑ‚Ð¾.Ñ€Ñ„',
            'first.last@phplÃ­st.com',
        ];
        $asciiaddresses = [
...
EOT;
    ...
    public function testHtmlIso8859()
    {
        ...
        // This is the string 'Ã©Ã¨Ã®Ã¼Ã§Ã…Ã±Ã¦ÃŸ' in ISO-8859-1, base-64 encoded
    ...
    {
        $this->Mail->isHTML(true);
        $this->Mail->Subject .= ': UTF-8 HTML ÐŸÑƒÑ�Ñ‚Ð¾Ðµ Ñ‚ÐµÐ»Ð¾ Ñ�Ð¾Ð¾Ð±Ñ‰ÐµÐ½Ð¸Ñ�';
        $this->Mail->CharSet = 'UTF-8';
        $this->Mail->Body = <<<'EOT'
<html>
    <head>
...
EOT;
    ...
    public function testConfirmReadingTo()
    {
        ...
        $this->Mail->ConfirmReadingTo = 'test@franÃ§ois.ch';  //Address with IDN
    ...
    public function testConvertEncoding()
    {
        ...
        $domain = '@' . mb_convert_encoding('franÃ§ois.ch', 'ISO-8859-1', 'UTF-8');
    ...
    public function testDuplicateIDNRemoved()
    {
        ...
        $this->assertFalse($this->Mail->addAddress('test@franÃ§ois.ch'));
        $this->assertTrue($this->Mail->addAddress('test@FRANÃ‡OIS.CH'));
        $this->assertFalse($this->Mail->addAddress('test@FRANÃ‡OIS.CH'));
        $this->assertTrue($this->Mail->addAddress('test@xn--franois-xxa.ch'));
        $this->assertFalse($this->Mail->addAddress('test@xn--franois-xxa.ch'));
        $this->assertFalse($this->Mail->addAddress('test@XN--FRANOIS-XXA.CH'));
        $this->assertTrue($this->Mail->addReplyTo('test+replyto@franÃ§ois.ch'));
        $this->assertFalse($this->Mail->addReplyTo('test+replyto@franÃ§ois.ch'));
        $this->assertTrue($this->Mail->addReplyTo('test+replyto@FRANÃ‡OIS.CH'));
        $this->assertFalse($this->Mail->addReplyTo('test+replyto@FRANÃ‡OIS.CH'));
