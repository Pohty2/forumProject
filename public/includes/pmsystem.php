...
define("password", "test123"); // your database password
define("database", "phpcrudsample"); // your database
define("TBL_USERS","tb_user"); // your users database table
...
class Private_messaging_system{
    ...
    public static function fillmsg($row){
        $msg=new Message();
        $msg->id=$row["id"];
        $msg->user_to=$row["user_to"];
        $msg->user_from=$row["user_from"];
    ...
    public function send_message($to, $message, $subject, $respond = 0){
        $from = $_SESSION['id']; // ID of a user sending a message
    ...
    public function get_all_messages(){
        ...
        $sql="select * from messages where user_to ='$id' OR user_from ='$id'";
        $result = $conn->query($sql);
        if ($result->num_rows > 0) {
            while($row = $result->fetch_assoc()){
                $msg=self::fillmsg($row);
                $msgs[]=$msg;
            }
        }
        return $msgs;
